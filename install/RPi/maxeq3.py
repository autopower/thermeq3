...
class EQ3Data:
    ...
    def get_room_name(self, k):
        ...
        """
        return self.rooms[str(self.devices[k][3])][0]
    ...
    def cmd_c(self, line):
        ...
        if ord(es[0x04]) == 1:
            ...
            if dev_adr not in self.devices:
                self.devices.update({dev_adr: [0, 0, 0, 0, 0, 0, 0, 0, 0]})
            ...
            if valve_adr in self.devices:
                ...
                # HeatingThermostat (dev_type 1 or 2)
                    ...
                    if self.is_valid(valve_info):
                        ...
                        try:
                            ret_val = self.rooms[valve_room_id][3]
                        except KeyError:
                            pass
                        else:
                    ...
                    if valve_curr_temp == 0.0 and not self.set_zero:
                        if valve_adr in self.valves:
                            valve_curr_temp = self.valves[valve_adr][2]
                        else:
                            # if first read is zero, set 0.1 to indicate that value is not present
                            valve_curr_temp = 0.1
    ...
    def json_status(self):
        ...
        Return json in format
            {room_name: {valve_addr: {valve_name, valve_position, set_temp, current_temp, count_valve, ignore_until, status}}}
        :return: json
        """
        # devices = {addr: [type, serial, name, room, OW, OW_time, status, info, temp offset]}
