...
class Thermeq3Object(object):
    ...
    def prepare(self):
        ...
        # get values and put it in the bridge
        self._literal_process(br_data)

        # initialize variables again, if something is not initialized set defaults
        ...
        if mailer.check_credentials(self.setup):
            logmsg.update("Mail server and credentials OK.", 'I')
        else:
            logmsg.update("Mail server and/or credentials not working.", 'E')
    ...
    def save_bridge_control_values(self):
        for k, v in bridge.cw.iteritems():
            if v[2]:
                sv = v[3].split('.')
                obj_obj = eval("self." + sv[0])
    ...
    def set_bridge_control_values(self):
        ...
        bridge.put("ign", json.dumps(self.eq3.ignored_valves))
        bridge.put("profile", self.setup.selected_mode)
    ...
    def _do_thermeq(self):
        ...
        if eq3_result:
            ...
            if any("response" in e for e in eq3_error):
                self.queue_msg('M')
                self.status.update('e', "M response error!")
                msg = mailer.new_compose("m_response", "", "")
                mailer.send_email(self.setup, msg.as_string())
            else:
                self.queue_msg('E')
                self.status.update('e', "Timeout!")
            # flush error to log
            logmsg.update("".join(str(e) + " " for e in eq3_error), 'E')
        # and finally update bridge
        bridge.save(self.setup.bridge_file)
    ...
    def _do_beta(self):
        ...
        logmsg.update("Beta code start.", 'D')
        if self._is_beta():
            logmsg.update("Profiles start.", 'D')
            self._do_profiles()
            logmsg.update("Profiles stop.", 'D')
        logmsg.update("Beta code stop.", 'D')
    ...
    def _status_msg(self):
        ...
        :return: string
        """
        _log_str = self.status.actual + ", "
        if self.setup.preference == "per":
            _log_str += str(self.setup.valve_switch) + "%" + " at " + str(self.setup.valve_num) + " valve(s)."
        elif self.setup.preference == "total":
            _log_str += "total value of " + str(self.setup.total_switch) + "."
        return _log_str
    ...
    @staticmethod
    def update_total_heat(value):
        try:
            _total_heat_time = float(bridge.try_read("tht"))
        except Exception:
            logmsg.update("Can't read total heat time value from bridge!", 'E')
        finally:
            bridge.put("tht", value + _total_heat_time)
            logmsg.update("Total heat time updated to " + str(value + _total_heat_time), 'I')

    def create_new_heat_key(self):
        # create the new key
        _nw = datetime.datetime.date(datetime.datetime.now()).strftime("%d-%m-%Y")
        self.var.ht.update({_nw: [0, time.time()]})
        logmsg.update("Creating new daily key: " + str(_nw) + ": " + str(self.var.ht[_nw]))
    ...
    def delete_old_ht_key(self, nw):
        _temp_ht = self.var.ht
        _temp_ht.pop(nw)
        for k in _temp_ht.keys():
            self.update_total_heat(_temp_ht[k][0])
            del self.var.ht[k]
    ...
    def update_counters(self, heat_start):
        ...
        _nw = datetime.datetime.date(datetime.datetime.now()).strftime("%d-%m-%Y")
        tm = time.time()
        # call home with AppLive event
        support.call_home("applive")

        # is there a key for today?
        if _nw in self.var.ht:
            # if there is a today key and len > 1, there must be also some old key
            if len(self.var.ht) > 1:
                self.delete_old_ht_key(_nw)
            if heat_start:
                self.var.ht[_nw][1] = tm
            elif self.var.heating:
                daily_heat_sum = int(self.var.ht[_nw][0] + (tm - self.var.ht[_nw][1]))
                logmsg.debug("Daily heat sum=", daily_heat_sum)
                self.var.ht[_nw] = [daily_heat_sum, time.time()]
                bridge.put("ht", self.var.ht)
            ...
            if len(self.var.ht) > 0:
                ...
                for k in self.var.ht.keys():
                    _v = self.var.ht[k]
                    self.update_total_heat(_v[0])
                    logmsg.update(str(k) + " daily heating summary: " + str(datetime.timedelta(seconds=_v[0])), 'I')
                    logmsg.update("Deleting old daily key: " + str(k), 'D')
                    del self.var.ht[k]
                self.export_csv("close")

            self.create_new_heat_key()
            # bridge.save(self.setup.bridge_file)
        bridge.put("ht", self.var.ht)
                    ...
                    if _pcw_v[1]:
                        ...
                        try:
                            ...
                            try:
                                ...
                                # put value into the bridge
                                bridge.try_put(k, value)
        except AttributeError:
            logmsg.update('Bridge file has attribute error. May be file is missing.')
    ...
    def update_ignores_2sit(self):
        ...
        w_mode = bridge.try_read("wref")
        self.var.situation = weather.get_weather(str(w_mode).upper(),
                                                 {"woe_id": self.setup.yahoo_location,
                                                  "owm_id": self.setup.owm_location,
                                                  "owm_api_key": self.setup.owm_api_key})
    ...
    def write_strings(self):
        ...
        # second web
        # and bridge variable
        bridge.put("sys", self.eq3.json_status())
        # and dump whole system
        bridge.put("dump", self.eq3.json_dump())
    ...
    def check_var(self):
        ...
        if self.setup.valve_num > len(self.eq3.valves):
            ...
            msg = mailer.new_compose("valve_number", str(len(self.eq3.valves)), str(self.setup.valve_num))
            mailer.send_email(self.setup, msg.as_string())
        ...
        if self.setup.svpnmw > 100:
            logmsg.update("Single valve switch position over 100%! Please check brige svpnmw (set to " + str(self.setup.svpnmw) + ") variable!")
