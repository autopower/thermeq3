...
class setup(object):
    def __init__(self):
        ...
        self.github = "https://raw.github.com/autopower/thermeq3/master/install"
...
def downloadFile(filename):
    ...
    try:
        ...
        try:
            ...
            errstr = "Problem during saving new version. File: " + stp.homedir + fbase + ". Error: " + str(
        ...
        if new_hash == "":
            ...
            if new_hash != t[3] and stp.version <= tmp_ver:
                ...
                if down_result:
                    ...
                    var.logger.error(
                        "Problem downloading new version. Result=" + str(down_result) + ", file=" + str(t[1]))
            else:
                return 1
...
def silence(key, isWin):
    ...
    if var.d_W[key][2] > stp.abnormalCount:
        var.logger.critical(
            "Abnormal count of warnings for device [" + str(key) + "], name [" + str(stp.devices[key][2]) + "]")
        var.d_W[key][2] = 0
    return 0
...
def exportCSV(onoff):
    if onoff == "init":
        if os.path.exists(stp.csv_log):
            os.rename(stp.csv_log,
                      stp.place + stp.devname + "_" + time.strftime("%Y%m%d-%H%M%S", time.localtime()) + ".csv")
        try:
...
def setIgnoredValves(key):
    ...
    for k, v in stp.devices.iteritems():
        ...
        if v[0] == 1 and v[3] == room and k not in var.d_ignore:
            ...
            var.logger.debug("Ignoring valve " + str(k) + " until " + time.strftime("%d/%m/%Y %H:%M:%S",
                                                                                    time.localtime(var.d_ignore[k])))
